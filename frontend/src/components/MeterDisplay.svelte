...
<div class="card bg-base-200 border border-base-300 shadow-sm px-4 py-3">
  <div class="flex items-center justify-between mb-2">
    <span class="text-sm font-semibold text-base-content/50 uppercase tracking-widest">仪表</span>
    <input type="checkbox" class="toggle toggle-success" checked={meterOn} onchange={(e)=>toggle(e.target.checked)} />
  </div>
  <div class="grid grid-cols-[2fr_1fr_1fr_1fr] gap-3">
    ...
    <div class="min-w-0">
      <div class="flex justify-between items-baseline mb-0.5">
        <span class="text-sm font-semibold text-base-content/50 uppercase">{it.label}</span>
        <span class="font-mono text-sm font-semibold">{it.val}</span>
      </div>
      <div class="relative h-4 bg-base-300 rounded-sm overflow-hidden border border-base-300">
