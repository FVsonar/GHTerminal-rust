...
<div class="flex items-end gap-2 px-2.5 py-1.5 bg-base-200 border border-base-300 rounded-md">
  ...
  <div class="flex-1 min-w-0 flex flex-col">
    <span class="text-[13px] text-base-content/50 mb-0.5">显示</span>
    <select class="select select-sm select-bordered w-full text-[13px]" onchange={(e)=>sendCommand('set_display_mode',{mode:parseInt(e.target.value)})}>
      {#each DISPLAY_MODES as m,i}<option value={i}>{m}</option>{/each}
    </select>
  </div>
  <div class="flex-1 min-w-0 flex flex-col">
    <span class="text-[13px] text-base-content/50">REF {p.ref||10}</span>
    <input type="range" class="range range-sm range-primary" min="1" max="20" value={p.ref||10} oninput={(e)=>sendCommand('set_spectrum_ref',{value:parseInt(e.target.value)})} />
  </div>
  <div class="flex-1 min-w-0 flex flex-col">
